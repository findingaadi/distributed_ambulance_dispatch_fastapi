...
def view_assignment(ambulance_id: int, db: Session = Depends(get_db)):
    assignment = db.query(Assignment).filter(Assignment.ambulance_id == ambulance_id, Assignment.status == "pending").first()
    if not assignment:
